...
export async function getTurboBalanceWithShared(turbo) {
  ...
  // balance has: winc (native), receivedApprovals: [{ payingAddress, approvedWincForSpend }]
  
  const nativeWinc = BigInt(balance.winc);
  const sharedWinc = (balance.receivedApprovals || [])
    .reduce((sum, a) => sum + BigInt(a.approvedWincForSpend), 0n);
