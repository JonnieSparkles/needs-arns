...
export function generateMediaUploadTags(contentType, customAppName = null) {
  ...
  const tags = [
    ...
    { name: 'App-Name', value: customAppName || config['App-Name'] },
    { name: 'App-Version', value: '1.0.0' }
  ];

  // Add custom tags from config
  if (config.Title) tags.push({ name: 'Title', value: config.Title });
  if (config.Description) tags.push({ name: 'Description', value: config.Description });

  return tags;
}

// Generate tags for manifest uploads
export function generateManifestUploadTags(customAppName = null) {
  const config = loadUploadTagsConfig();
  const tags = [
...
export async function uploadManifest(manifestBuffer, appName = 'NeedsArNS', jwk = null) {
  try {
    ...
    // Generate tags from configuration
    const tags = generateManifestUploadTags(appName);
