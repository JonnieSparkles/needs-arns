...
export async function getUserTweets(twitterClient, userId, sinceId = null, options = {}) {
  try {
    ...
    if (process.env.DEBUG_TWITTER === 'true') {
      console.log('📋 Raw API response keys:', Object.keys(response));
      console.log('📋 _realData:', JSON.stringify(response._realData, null, 2));
    }

    // Check for API errors in response
    if (response._realData?.errors) {
      console.warn('⚠️ Twitter API returned errors:', JSON.stringify(response._realData.errors, null, 2));
    }
    ...
    if (meta.result_count !== undefined) {
      console.log(`   API result_count: ${meta.result_count}, newest_id: ${meta.newest_id || 'none'}`);
    }
...
/**
 * Extract quoted tweet ID from a tweet
 * Checks both referenced_tweets field and URL entities for quote tweet links
 * @param {Object} tweet - Tweet object from API
 * @returns {string|null} The quoted tweet ID or null if not a quote tweet
 */
export function extractQuotedTweetId(tweet) {
  if (!tweet) return null;

  // Method 1: Check referenced_tweets for quote type
  if (tweet.referenced_tweets && Array.isArray(tweet.referenced_tweets)) {
    ...
    if (quotedRef?.id) {
      console.log(`   📝 Found quoted tweet via referenced_tweets: ${quotedRef.id}`);
      return quotedRef.id;
    }
  }

  // Method 2: Check URL entities for x.com/twitter.com status links
  // Some quote tweets (especially via Grok shares) don't have referenced_tweets
  // but have the quoted post URL in entities.urls[].title or expanded_url
  if (tweet.entities?.urls) {
    for (const urlEntity of tweet.entities.urls) {
      // Check the title field (Grok shares put the actual tweet URL here)
      const titleMatch = urlEntity.title?.match(/(?:x\.com|twitter\.com)\/\w+\/status\/(\d+)/i);
      if (titleMatch) {
        console.log(`   📝 Found quoted tweet via URL entity title: ${titleMatch[1]}`);
        return titleMatch[1];
      }

      // Check expanded_url for direct quote tweets
      const expandedMatch = urlEntity.expanded_url?.match(/(?:x\.com|twitter\.com)\/\w+\/status\/(\d+)/i);
      if (expandedMatch) {
        // Make sure it's not the same tweet (self-reference) or a media link
        if (expandedMatch[1] !== tweet.id &&
            !urlEntity.expanded_url.includes('/photo/') &&
            !urlEntity.expanded_url.includes('/video/')) {
          console.log(`   📝 Found quoted tweet via expanded_url: ${expandedMatch[1]}`);
 ...
 * @param {Object} tweet - Tweet object from API
 * @returns {boolean} True if the tweet quotes another tweet
 */
export function isQuoteTweet(tweet) {
  return extractQuotedTweetId(tweet) !== null;
}
