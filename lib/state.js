...
export function saveProcessedState(processedMentions, sinceId, processedDetails = {}, filePath = 'processed_mentions.json') {
  try {
    const state = {
      ...
      processedDetails: processedDetails, // { mentionId: { undername, txId, success, timestamp, mentionUsername } }
      lastSinceId: sinceId,
  ...
  try {
    ...
    if (replyTweetId) {
      state.processedDetails[replyTweetId] = {
        mentionUsername: mentionDetails.mentionUsername || mentionDetails.username || 'manual',
