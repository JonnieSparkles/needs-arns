...
function createIndexEntry(archiveResult, tweet, invokingUser = null) {
  const entry = {
    ...
    authorProfileImage: archiveResult.metadata?.display?.profileImageUrl || null
  };
  ...
  if (invokingUser) {
    ...
    entry.invokingProfileImage = invokingUser.profile_image_url || null;
  }
...
/**
 * Count how many posts a user has archived (as invoker)
 * @param {Object} index - The index object with posts array
 * @param {string} username - The invoking username to count
 * @returns {number} Count of posts archived by this user
 */
export function countPostsByInvoker(index, username) {
  if (!index?.posts || !username) return 0;
  return index.posts.filter(p =>
    p.invokingUsername?.toLowerCase() === username.toLowerCase()
  ).length;
}
 ...
 * @param {Object} options - Additional options { archiveCount, invokingUsername }
 * @returns {string} Reply message
 */
export function generateWatchReplyMessage(postId, arnsName, account = null, options = {}) {
  const { archiveCount, invokingUsername } = options;

  // Use success-baseposting template for the baseposting account
  if (account?.arnsName === 'baseposting' || arnsName === 'baseposting') {
    const message = renderTemplate('success-baseposting', {
      postId,
      archiveCount: archiveCount || '1',
      invokingUsername: invokingUsername || 'unknown'
    });
    if (message) {
      return message;
    }
