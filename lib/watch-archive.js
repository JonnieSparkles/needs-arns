 ...
 */
export function buildWatchMetadataObject(tweet, account, includes, mediaArray = []) {
 ...
 */
export async function archiveWatchedPost(tweet, account, includes, jwk, templateTxId) {
  ...
  try {
    ...
    const metadataObj = buildWatchMetadataObject(tweet, account, includes, mediaArray);
    ...
    // 4. Generate and upload manifest
    ...
    // 5. Save local archive copy
    saveWatchPostArchive(metadataObj, account.arnsName);

    console.log(`   ✅ Post archived: manifest ${manifestTxId}`);
