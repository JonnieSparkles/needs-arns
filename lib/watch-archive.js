 ...
 */
export function buildWatchMetadataObject(tweet, account, includes, mediaArray = [], quotedTweetData = null, quotedMediaArray = []) {
  ...
  const metadataObj = {
    metadata: {
      ...
      archiveVersion: '2.1.0' // Bumped for quote tweet support
    },
    rawApiResponse: {
 ...
 */
export async function archiveWatchedPost(tweet, account, includes, jwk, templateTxId, twitterClient = null) {
  const postId = tweet.id;
  console.log(`\n📦 Archiving post ${postId} from @${account.twitterUsername}`);
  ...
  try {
    ...
    // 3. Build metadata object (with uploaded quoted media)
    const metadataObj = buildWatchMetadataObject(tweet, account, includes, mediaArray, quotedTweetData, quotedMediaArray);
 ...
 * @param {Object} tweet - Original tweet object
 */
export function addPostToIndexInMemory(index, account, archiveResult, tweet) {
  // Update metadata if not set
  if (!index.metadata.watchedAccount) {
  ...
  if (existingIdx >= 0) {
    ...
    index.posts[existingIdx] = createIndexEntry(archiveResult, tweet);
  } else {
    // Add to beginning (newest first)
    index.posts.unshift(createIndexEntry(archiveResult, tweet));
  }
}
 ...
 */
function createIndexEntry(archiveResult, tweet) {
  return {
 ...
 */
export function generateWatchReplyMessage(postId, arnsName) {
