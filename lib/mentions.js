...
export async function handleSuccess(twitterClient, mentionId, undername, ownerArnsName, txId, isUploadedMedia) {
  ...
  if (!msg) {
    // Fallback if template loading fails
    const fallbackMsg = `🎉 ${undername}_${ownerArnsName}.ar.io → ${txId}`;
