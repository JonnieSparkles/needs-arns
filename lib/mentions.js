...
export function extractCommandFromMention(mentionText) {
  ...
  // Strict pattern: @NeedsArNS [optional whitespace] (assign|archive|name this) [whitespace] undername
  ...
  // - (assign|archive|name\s+this) - Command verbs (note: "name this" is required, not just "name")
  // - \s+ - Required whitespace
  // - ([a-z0-9_-]{1,63}) - Undername (1-63 chars, lowercase alphanumeric, dash, underscore)
  // - \b - Word boundary to ensure complete match
  const strictPattern = /@NeedsArNS\b\s*(assign|archive|name\s+this)\s+([a-z0-9_-]{1,63})\b/i;
  ...
  const verb = m[1].toLowerCase().replace(/\s+/g, ' ').trim(); // 'assign', 'archive', or 'name this'
  ...
  if (!isValidUndername(undername)) {
    console.log(`🚫 Invalid undername format: "${undername}"`);
    return null;
  }
  ...
  if (verb === 'archive') {
    type = 'archive';
  } else if (verb === 'name this') {
    type = 'name';
  } else if (verb === 'assign') {
    type = 'assign';
  } else {
    // Safety check - if verb doesn't match expected patterns, reject
    console.log(`🚫 Unknown command verb: "${verb}"`);
    return null;
  }
