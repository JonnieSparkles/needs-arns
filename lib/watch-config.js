...
const DEFAULTS = {
  ...
  enabled: true,
  replyToPost: true,
  sourceType: 'timeline' // 'timeline' or 'search'
};

// Valid source types for watch accounts
const VALID_SOURCE_TYPES = ['timeline', 'search'];
...
function validateAccount(account, index) {
  ...
  // Validate sourceType (optional, defaults to 'timeline')
  const sourceType = account.sourceType || DEFAULTS.sourceType;
  if (!VALID_SOURCE_TYPES.includes(sourceType)) {
    throw new Error(`${prefix}: sourceType must be one of: ${VALID_SOURCE_TYPES.join(', ')}`);
  }
  ...
  // twitterUserId is required for timeline-based accounts, optional for search-based
  if (sourceType === 'timeline') {
    if (!account.twitterUserId || typeof account.twitterUserId !== 'string') {
      throw new Error(`${prefix}: twitterUserId is required for timeline-based accounts`);
    }
  }

  // searchQuery is required for search-based accounts
  if (sourceType === 'search') {
    if (!account.searchQuery || typeof account.searchQuery !== 'string') {
      throw new Error(`${prefix}: searchQuery is required for search-based accounts`);
    }
  }
  ...
  // Validate optional postTemplateTxId (must be valid Arweave TXID format)
  if (account.postTemplateTxId) {
    if (typeof account.postTemplateTxId !== 'string' || !/^[a-zA-Z0-9_-]{43}$/.test(account.postTemplateTxId)) {
      throw new Error(`${prefix}: postTemplateTxId must be a valid Arweave transaction ID (43 base64url characters)`);
    }
  }
  ...
  return {
    ...
    twitterUserId: account.twitterUserId || null, // null for search-based accounts
    ...
    sourceType,
    searchQuery: account.searchQuery || null, // null for timeline-based accounts
    filtering,
    postTemplateTxId: account.postTemplateTxId || null // null uses global default
  };
}
