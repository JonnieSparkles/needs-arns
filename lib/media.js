...
export async function getMediaUrls(tweetData, includes) {
  ...
  // Check tweet entities for video URLs to detect mis-labeled media types
  const tweetUrls = tweetData.entities?.urls || [];
  const hasVideoUrl = tweetUrls.some(url => 
    url.expanded_url?.includes('/video/') || 
    url.display_url?.includes('video') ||
    url.media_key // If URL has media_key, it's attached media
  );
  ...
  const results = await Promise.all(mediaKeys.map(async (key) => {
    ...
    // Detect video type: check if URL entity indicates video or if media_key format suggests video
    // Video media_keys often start with "13_" or "7_", photos with "3_"
    const isVideoByKey = key.startsWith('13_') || key.startsWith('7_');
    const correspondingUrl = tweetUrls.find(url => url.media_key === key);
    const isVideoByUrl = correspondingUrl?.expanded_url?.includes('/video/');
    
    // Override type if Twitter mis-labeled it
    let actualType = mediaObj.type;
    if ((isVideoByKey || isVideoByUrl) && mediaObj.type === 'photo') {
      console.log(`🔧 Correcting media type from 'photo' to 'video' (detected by key: ${isVideoByKey}, url: ${isVideoByUrl})`);
      actualType = 'video';
    }
    
    // Check if it's actually an animated_gif based on variants
    if (mediaObj.variants && mediaObj.variants.some(v => v.content_type?.includes('gif'))) {
      actualType = 'animated_gif';
    }
    
    console.log(`🔍 Processing media: type=${mediaObj.type} -> ${actualType}, hasUrl=${!!mediaObj.url}, hasPreview=${!!mediaObj.preview_image_url}, hasVariants=${!!mediaObj.variants}`);
    
    // For videos and animated GIFs, try to get the highest quality variant
    let bestUrl = mediaObj.url || mediaObj.preview_image_url;
    
    if ((actualType === 'video' || actualType === 'animated_gif') && mediaObj.variants) {
    ...
    // Return the best URL available with corrected type
    return {
      url: bestUrl,
      type: actualType, // Use the corrected type
...
export async function processMediaFromTweet(tweetData, includes, uploadToArweave) {
  ...
  console.log(`📸 Processing ${mediaUrls.length} media attachment(s)`);
  
  try {
    // Process all media attachments
    const allMediaResults = await Promise.all(mediaUrls.map(async (media, index) => {
      try {
        console.log(`📸 Processing ${media.type} media ${index + 1}/${mediaUrls.length}: ${media.url}`);
        ...
        console.log(`✅ Media ${index + 1} uploaded to Arweave: ${txId}`);
        ...
        console.error(`❌ Failed to upload media ${index + 1}:`, error);
        return {
          success: false,
          error: error.message,
          index
    ...
    const failedMedia = allMediaResults.filter(m => !m.success);
    if (failedMedia.length > 0) {
      console.warn(`⚠️ ${failedMedia.length}/${allMediaResults.length} media uploads failed`);
    }
    
    // Filter to successful uploads only
    const successfulMedia = allMediaResults.filter(m => m.success);
    
    if (successfulMedia.length === 0) {
      return { success: false, error: 'upload_failed', message: 'All media uploads failed' };
    }
    ...
    return {
      ...
      // For backward compatibility, include first media txId
      txId: successfulMedia[0].txId
    };
  } catch (uploadError) {
    console.error(`❌ Failed to process media:`, uploadError);
