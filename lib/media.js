...
export async function getMediaUrls(tweetData, includes) {
  ...
  const results = await Promise.all(mediaKeys.map(async (key) => {
    ...
    // If media object is missing from includes (shouldn't happen with proper API calls)
    // Fallback: try to detect from media_key prefix pattern
    if (!mediaObj) {
      console.warn(`⚠️ Media object not found for key ${key}, using fallback detection`);
      // Video media_keys often start with "13_", "7_", or "16_", photos with "3_"
      const isVideoByKey = key.startsWith('13_') || key.startsWith('7_') || key.startsWith('16_');
      ...
      return {
        url: correspondingUrl?.expanded_url || '',
        type: (isVideoByKey || isVideoByUrl) ? 'video' : 'photo',
        width: null,
        height: null,
        alt_text: '',
        media_key: key
    ...
    // Trust Twitter's type field when we have the media object
    ...
    // Return the media with type from Twitter API (or detected animated_gif)
    return {
      url: bestUrl,
      type: actualType,
